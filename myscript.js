...
function initialise() {
  for (let i = 0; i < usr.length; i++) {
    ...
    <p>${usr[i].firstName + " " + usr[i].lastName}</p></div>`;
...
function drop(event) {
  ...
  rcMap.set(`${event.target.id}`, true);
  console.log(
    `${event.target.id}` + " " + rcMap.get(`${event.target.id}`)
  );
}
